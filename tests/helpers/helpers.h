...
std::vector<Patch> ParsePatchesFromArgs(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        if (argv[i] != nullptr)
        {
            ...
            {
                std::cout << "Patch: " << patch_pair[0] << " -> "
                          << patch_pair[1] << std::endl;
...
mcctp::PatchMap ParsePatchMapFromPatches(const std::vector<Patch> &patches)
{
    ...
    else
    {
        ...
        for (const auto &patch : patches)
        {
            ...
            // mcctp::PatchSource patch_src(src_name, src_name,
            // mcctp::SourceType::Internal); if (patch_src.GetType() !=
            // mcctp::SourceType::Invalid) {
            //}

            if (res1.has_value() && res2.has_value())
            {
                mcctp::PatchSource patch_src(src_name, src_name,
                                             mcctp::SourceType::Internal);
...
bool PatchAndWriteBins(const std::vector<Patch> &patches)
{
    ...
    else
    {
        ...
        for (const auto &patch : patches)
        {
            ...
            const auto target_pack =
                inst->GetTexturePackFlagFromResourceName(dst);
...
void PrintPatchMap(const mcctp::PatchMap &map)
{
    ...
    for (const auto &entry : map)
    {
        ...
        std::cout << std::to_string(count) << " ("
                  << mcctp::SourceTypeToString.at(src.GetType()) << ")"
