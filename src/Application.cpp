...
Application::Application(const ApplicationSpecification &applicationSpecification) {
  m_Window = std::make_unique<BorderlessWindow>(applicationSpecification.Width, applicationSpecification.Height);
...
void Application::Run() {
  ...
  while (!done) {
    ...
    static auto render = [&]() {
        StartNewAppFrame();
        {
            ...
            ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav;
            {
                if (ImGui::Begin("Borderless Settings", 0, window_flags))
                {
                    ...
                    if (window_mode.update()) m_Window->set_borderless(window_mode.m_Value);
                }
                ImGui::End();
            }
        ...
        if (performance_mode) {
            while (dt <= desired_frame_time) {
                ...
                // Yeah yeah giving up cpu time is 'spensive but the cpu load reduction here is epic
                LONGLONG ns = ((((((1000.0f / (desired_fr * 1000.0f)) - dt) * 1000000000LL) / 100) * 100));
                if (ns > 0) {
    ...
    if (!set_render) {
        m_Window->set_render_callback(render); //render when we are stuck in the message loop doing wm_sizing and wm_move
        set_render = true;
    }
...
Application *Application::Instance() { return (Application *)g_Instance; }
