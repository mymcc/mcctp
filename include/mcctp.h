...
struct Result {
  ...
  public:
    ...
    const ResultType &GetType(void) const {
        return Type;
    }
...
struct FileMapping {
  ...
  public:
    explicit FileMapping(std::wstring path, uint64_t file_flags = GENERIC_READ | GENERIC_WRITE,
...
enum TexturePackFlags {
    ...
    Skulls = 1 << 11,
    SPMapPreview = 1 << 12,
    All = (Controller | Emblems | GlobalUI | Hopper | InGameChapter | LargeAvatar | Levels |
           Loading | MainMenuAndCampaign | MainMenu | Medals | Skulls | SPMapPreview),
    None = 1 << 13,
};
...
static const FileMapping &GetPerm(const TexturePack &tp) {
    return std::get<0>(tp);
}
static const FileMapping &GetTemp(const TexturePack &tp) {
    return std::get<1>(tp);
}
static TexturePackType GetType(const TexturePack &tp) {
    return std::get<2>(tp);
}
...
struct TexturePackField {
  ...
  public:
    ...
    bool HasFlag(TexturePackFlags flags) {
        return (Value & (int)flags) == (int)flags;
    }
    bool HasAnyFlag(TexturePackFlags flags) {
        return (Value & (int)flags) != 0;
    }
...
enum class TexturePackInjectFlags { Strict, None };
...
struct PatchSource {
  ...
  public:
    ...
    const SourceType &GetType(void) const {
        return Type;
    }
    const SourceProvider &GetProvider(void) const {
        return Provider;
    }
...
class ctx {
  public:
    ...
    size_t GetMappedTexturePackCount(void) const {
        return m_TexturePackFileMap.size();
    }
    ...
    size_t GetIndexedTexturePackCount(void) const {
        return m_TexturePackIndexMap.size();
    }
    bool IsTexturePackIndexed(TexturePackFlags flag) const;

    bool DumpTexturePack(TexturePackFlags flag,
                         DumpFormatFlags format_flag = DumpFormatFlags::Native,
  ...
  private:
    ...
    ResourceToTextureMap m_ResourceToTexturePackMap; /* Reverse resource index for a mapped
                                                        texture pack (reduces patch time
                                                        complexity) */
