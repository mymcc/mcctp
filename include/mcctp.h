...
namespace mcctp {
	...
	public:
		...
		~FileMapping() {
			if (lpMap != NULL) UnmapViewOfFile(lpMap);
	...
	public:
		...
		bool MemoryMapTexturePack(TexturePackFlags flag, std::wstring texture_pack_basename) {
			try {
				m_TexturePackFileMap.insert({ flag, std::move(FileMapping(m_PathPrefix.wstring() + L'\\'
					+ texture_pack_basename + L".perm.bin"))});
				return true;
			}
	...
	static bool MemoryMapTexturePacks(void)
	{
		...
		for (uint8_t i = 0; i < TexturePackCount; ++i) {
			...
			if (field.HasFlag(flag)) {
				...
				bool success = ctx::Instance()->MemoryMapTexturePack(flag, converter().from_bytes(FlagToBasename.at(flag)));
			}
		}
		return true;
	}
